...
@section('content')
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="modalDelete">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('seller.availability.destroy', ['id' => '#']) }}" method="post" id="formDelete">
                	{{ method_field('DELETE') }}
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                        <h4 class="modal-title">Hapus Penyesuaian</h4>
                    </div>

                    <div class="modal-body">
                        <p id="del-success">Anda yakin ingin menghapus Penyesuaian ?</p>
                    </div>
                    <div class="modal-footer">
    ...
    <script>
        jQuery(document).ready(function($){
            var table = $('#data_table').DataTable({
                ...
                "lengthChange": true,
                "ajax": {
                    "url": "{{ route('seller.availability.index') }}",
                    "type": "POST",
                    "data" : {}
                },
                "language": {
                    ...
                    },
                    {
                        render : function(data, type, row){
                            if (row.status == "pending") {
                                return  '<a href="{{ route('seller.availability.index') }}/'+ row.id +'" class="btn btn-xs btn-info"><i class="fa fa-eye"></i></a> &nbsp' +
                                ...
                                return  '<a href="{{ route('seller.availability.index') }}/'+ row.id +'" class="btn btn-xs btn-info"><i class="fa fa-eye"></i></a>';
                            }
                        },
                        "width": "10%",
                        "orderable": false,
            ...
            $('#btnAdd').click(function () {
                $('#formAdd')[0].reset();
                $('#formAdd .modal-title').text("Tambah Penyesuaian");
                ...
                url = '{{ route("seller.availability.store") }}';
            ...
            $('#data_table').on('click', '.edit-btn', function(e){
                $('#formAdd div.form-group').removeClass('has-error');
                $('#formAdd .help-block').empty();
                $('#formAdd .modal-title').text("Ubah Penyesuaian");
                $('#formAdd')[0].reset();
                ...
                url = '{{ route("seller.availability.index") }}' + '/' + aData.id;
