...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-default">
                ...
                <div class="box-header with-border">
                    <h3 class="box-title pull-left">Produk Rakitan</h3>
                ...
                <div class="box-body">
                    <div class="table-responsive">
                        <table id="data_table_rakitan" class="table table-striped table-bordered table-hover nowrap dataTable">
                            <thead>
                                <tr>
                                    <th>Kode Produk</th>
                                    <th>Nama Produk</th>
                            ...
                            <tbody>
                                @foreach ($product_assembly as $item)
                                    <tr>
                                        ...
                                        @if ($data->status == "pending")
                                        <td>
                                            <a href="#" data-productassemblytid="{{ $item->id }}" class="btn btn-xs btn-danger delete-btn"><i class="fa fa-trash"></i></a>
                                        </td>
                                        @endif
                                    </tr>
                                @endforeach
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-default">
                ...
                <div class="box-body">
                    <div class="table-responsive">
                        <table id="data_table_jadi" class="table table-striped table-bordered table-hover nowrap dataTable">
                            <thead>
                                <tr>
                                    <th>Kode</th>
                                    <th>Produk</th>
                            ...
                            <tbody>
                                @foreach ($detail_assembly as $item)
                                    <tr>
                                        <td>{{ $item->code }}</td>
                                        <td>{{ $item->product->name }}</td>
                                        <td>{{ $item->quantity }}</td>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="modalDelete">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('seller.product-assembly.destroy', ['id' => '#']) }}" method="post" id="formDelete">
                    {{ method_field('DELETE') }}
                    <div class="modal-header">
                        <h4 class="modal-title">Hapus Produk Rakitan</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    ...
                    <div class="modal-body">
                        <p id="del-success">Anda yakin ingin menghapus Produk Rakitan?</p>
                    </div>
                    <div class="modal-footer">
    ...
    <!-- add and edit -->
    <div class="modal fade" tabindex="-1" role="dialog" id="modalAddJadi">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="#" method="post" id="formAddJadi" enctype="multipart/form-data" autocomplete="off">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="#" method="post" id="formDone">
                    <input type="hidden" name="assembly_id" id="assembly_id" value="{{ $data->id }}">
                    <div class="modal-header">
                        <h4 class="modal-title">Selesai Perakitan</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    ...
                    <div class="modal-body">
                        <p id="del-success">Anda yakin ingin menyelesaikan Perakitan ?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default pull-left" data-dismiss="modal">
                            Tidak
    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('#btnAdd').click(function () {
                $('#formAdd')[0].reset();
                $('#formAdd .modal-title').text("Tambah Produk Rakitan");
                ...
                url = '{{ route("seller.product-assembly.store") }}';
            ...
            // Delete
            $('#data_table_rakitan').on('click', '.delete-btn' , function(e){
                url =  $('#formDelete').attr('action').replace('#', $(this).data('productassemblytid'));
            ...
            // add
            $('#btnAddJadi').click(function () {
                $('#formAddJadi')[0].reset();
                $('#formAddJadi .modal-title').text("Tambah Produk Jadi");
                $('#formAddJadi div.form-group').removeClass('has-error');
                $('#formAddJadi .help-block').empty();
                $('#formAddJadi button[type=submit]').button('reset');

                $('#formAddJadi input[name="_method"]').remove();
                url = '{{ route("seller.detail-assembly.store") }}';

                $('#modalAddJadi').modal('show');
            });

            $('#formAddJadi').submit(function (event) {
                event.preventDefault();
                $('#formAddJadi div.form-group').removeClass('has-error');
                $('#formAddJadi .help-block').empty();
                $('#formAddJadi button[type=submit]').button('loading');

                var formData = new FormData($("#formAddJadi")[0]);
                ...
                $.ajax({
                    ...
                    error: function(response){
                        if (response.status === 422) {
                            // form validation errors fired up
                            var error = response.responseJSON.errors;
                            var data = $('#formAddJadi').serializeArray();
                            $.each(data, function(key, value){
                                if( error[data[key].name] != undefined ){
                                    console.log(data[key].name);
                                    var elem;
                                    if( $("#formAddJadi input[name='" + data[key].name + "']").length )
                                        elem = $("#formAddJadi input[name='" + data[key].name + "']");
                                    else if( $("#formAddJadi select[name='" + data[key].name + "']").length )
                                        elem = $("#formAddJadi select[name='" + data[key].name + "']");
                                    else
                                        elem = $("#formAddJadi textarea[name='" + data[key].name + "']");
                            ...
                            if(error['image'] != undefined){
                                $("#formAddJadi input[name='image']").parent().find('.help-block').text(error['image']);
                                $("#formAddJadi input[name='image']").parent().find('.help-block').show();
                                $("#formAddJadi input[name='image']").parent().parent().addClass('has-error');
                            }
                        }
                        else if (response.status === 400) {
                            // Bad Client Request
            ...
            $('#btnDone').click(function () {
                url = '{{ route("seller.detail-assembly.done") }}';
