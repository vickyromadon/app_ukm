...
@section('content')
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="modalDelete">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('seller.supplier.destroy', ['id' => '#']) }}" method="post" id="formDelete">
                	{{ method_field('DELETE') }}
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                    ...
                    <div class="modal-body">
                        <p id="del-success">Anda yakin ingin menghapus Pemasok ?</p>
                    </div>
                    <div class="modal-footer">
	...
	<script>
        jQuery(document).ready(function($){
            var table = $('#data_table').DataTable({
                ...
                "lengthChange": true,
                "ajax": {
                    "url": "{{ route('seller.supplier.index') }}",
                    "type": "POST",
                    "data" : {}
                },
                "language": {
            ...
            $('#btnAdd').click(function () {
                $('#formAdd')[0].reset();
                $('#formAdd .modal-title').text("Tambah Pemasok");
                ...
                url = '{{ route("seller.supplier.store") }}';
            ...
            $('#data_table').on('click', '.edit-btn', function(e){
                $('#formAdd div.form-group').removeClass('has-error');
                $('#formAdd .help-block').empty();
                $('#formAdd .modal-title').text("Ubah Pemasok");
                $('#formAdd')[0].reset();
                ...
                url = '{{ route("seller.supplier.index") }}' + '/' + aData.id;
